...
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle as RadixDialogTitle, DialogTrigger, DialogClose } from "@/components/ui/dialog"; // Renamed DialogTitle to RadixDialogTitle
...
const calculateDisplayScoresForTable = (
  ...
  roundActions.forEach(action => {
    ...
    else if (action.actionType === 'Binaan') binaan += 1;
  ...
  const calculateScoreBreakdownForDialog = useCallback(() => {
    ...
    ketuaActionsLog.forEach(action => {
      if (action.pesilatColor === 'merah') {
        ...
        } else if (action.actionType === 'Teguran' || action.originalActionType === 'Binaan') {
          teguranMerahCount++;
  ...
  const handleKetuaAction = async (pesilatColor: PesilatColorIdentity, actionTypeButtonPressed: KetuaActionType) => {
    ...
    try {
      ...
      if (actionTypeButtonPressed === 'Teguran') { actionTypeToLog = 'Teguran'; pointsToLog = TEGURAN_POINTS;
        ...
        if (binaanAsliCount < 2) { actionTypeToLog = 'Binaan'; pointsToLog = 0; } else { actionTypeToLog = 'Teguran'; pointsToLog = TEGURAN_POINTS; currentOriginalActionType = 'Binaan'; }
